...
export default function DocumentsPage() {
  ...
  const toggleApproved = async (id: string, current: boolean) => {
    ...
    fetchDocs();
    if (selected?.id === id) {
      setSelected((prev) => prev ? { ...prev, approved: !current } : null);
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-6 py-10">
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-24 bg-[#161616] rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : docs.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Document list grouped by customer */}
          <div className="lg:col-span-2 space-y-4">
            {Object.entries(grouped).map(([email, userDocs]) => {
              ...
              return (
                <div
                  key={email}
                  className="bg-[#161616] border border-[rgba(212,175,55,0.15)] rounded-2xl p-5"
                >
                  <div className="mb-4">
                    <p className="text-white font-semibold text-sm">{user?.full_name}</p>
                    <p className="text-[#71717A] text-xs">{user?.email}</p>
                    {user?.phone && (
                      <p className="text-[#71717A] text-xs">{user?.phone}</p>
                    )}
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    {userDocs.map((doc) => (
                      ...
                      >
                        <div className="relative h-28 bg-[#101010]">
                          <Image
            ...
            {selected ? (
              <div>
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-white font-bold text-base">
                    {formatType(selected.document_type)}
                  </h3>
                  <button
                ...
                {/* Large image preview */}
                <div className="relative h-48 bg-[#101010] rounded-xl overflow-hidden mb-4 border border-[#1F1F1F]">
                  <Image
                ...
                <div className="space-y-3 mb-5">
                  ...
                  <div className="flex items-center justify-between bg-[#101010] rounded-xl p-3">
                    ...
                    <span className={`text-xs font-bold ${
                      selected.approved ? "text-green-400" : "text-yellow-500"
                    }`}>
                ...
                <div className="space-y-2">
                  <button
                    onClick={() => toggleApproved(selected.id, selected.approved)}
              ...
              <div className="text-center py-12">
                <p className="text-[#71717A] text-sm">
                  Click a document to review and approve it
                </p>
              </div>
            )}
          </div>
